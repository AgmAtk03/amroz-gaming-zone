  ...
  return (
    <section className="border-b border-line">
      <div className="mx-auto max-w-5xl px-4 py-3 sm:px-6">
        <div className="chip-row flex gap-2 overflow-x-auto">
          {categories.map((cat) => (
            <a
              key={cat.id}
              href={cat.href}
              className="shrink-0 rounded-full border border-line bg-panel px-3 py-1.5 text-xs font-medium text-ink-soft"
            >
              {cat.label}
            </a>
          ))}
        ...
        <label className="mt-3 block">
          ...
          <input
            ...
            className="w-full rounded-full border border-line bg-panel px-4 py-2.5 text-sm outline-none focus:border-gold"
          />
        </label>
        {hits.length ? (
